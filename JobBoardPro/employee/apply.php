...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $cover_letter_file = $_FILES['cover_letter_file']['name'];
    $cover_letter_tmp = $_FILES['cover_letter_file']['tmp_name'];
    $cover_letter_ext = strtolower(pathinfo($cover_letter_file, PATHINFO_EXTENSION));
    ...
    if ($cover_letter_ext !== 'pdf' || $resume_ext !== 'pdf') {
        ...
        move_uploaded_file($cover_letter_tmp, "../uploads/" . $cover_letter_file);
        ...
        $stmt = $conn->prepare("INSERT INTO applications 
            (user_id, position_id, phone, address, birthday, age, gender, resume, other_docs, cover_letter_file) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        $stmt->bind_param("iisssisssss", $user_id, $position_id, $phone, $address, $birthday, $age, $gender, $resume, $docs, $cover_letter_file);
...
<form method="post" enctype="multipart/form-data">
    ...
    Upload Cover Letter (PDF only): <input type="file" name="cover_letter_file" accept="application/pdf" required><br><br>
